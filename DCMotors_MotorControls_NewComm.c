...
static long long left_steps = 0.0;	// This variable is used for tracking how many encoder pulses the left motor has rotated
static long long right_steps = 0.0;	// This variable is used for tracking how many encoder pulses the right motor has rotated
static long long top_steps = 0.0;	// This variable is used for tracking how many encoder pulses the top motor has rotated
static long long left_steps_last = 0.0;	// This variable is used for tracking how many encoder pulses the left motor had previously rotated
static long long right_steps_last = 0.0;// This variable is used for tracking how many encoder pulses the right motor had previously rotated
static long long top_steps_last = 0.0;	// This variable is used for tracking how many encoder pulses the top motor had previously rotated
...
const int	frequency  = 1500; 		// Let's check the kinematics this many times per second
const float GEARRATIO = 19.0*(3.0/4.0);		// The gear ratio of the gearhead on the DC motor
const float CPR = 100.0;			// The number of counts per revolution of the motor's encoder
static float kp = 100;				// Gain on the proportional error term
static float ki = 10;				// Gain on the integral error term
const float front_dist = 1.77;		// Perpendicular distance from axle axis to front of robot in inches
static float sum_error_r = 0.0;		// This is the integrated error for the right motor
static float sum_error_l = 0.0;		// This is the integrated error for the left motor
static float sum_error_t = 0.0;		// This is the integrated error for the top motor
...
{
    const float dtbase = 1.0/((float) frequency);
    const float convert = pi/(CPR*dtbase*GEARRATIO); 
    const float converttop = convert*(3.0/4.0);
    const float ticktime = 2.0/(80000000.0);
    float Vr = 0.0;
    float Vl = 0.0;
    float Vt = 0.0;
    float omega = 0.0;
    ...
    // and use that to set the PWM values
    float left_error = left_desired-left_speed;
    float right_error = right_desired-right_speed;
    float top_error = top_desired-top_speed;
    if(fabs(left_error) > ERROR_DEADBAND) SetSpeedLeft(left_error);
    if(fabs(right_error) > ERROR_DEADBAND) SetSpeedRight(right_error);
    if(fabs(top_error) > ERROR_DEADBAND) SetSpeedTop(top_error);
	
    // Now let's get the wheels speeds and convert them into translational velocities
    Vr = (D/2.0)*(right_speed);
    Vl = (D/2.0)*(left_speed);
    Vt = (0.835/2.0)*(top_speed);
...
void SetSpeedLeft(float error)  // motor speed in rad/s
{
    sum_error_l += error;
    float total_error = error*kp+sum_error_l*ki;
    unsigned int PWMVal = (unsigned int) (fabs(total_error));
...
void SetSpeedRight(float error)  // motor speed in rad/s
{
    sum_error_r += error;
    float total_error = error*kp+sum_error_r*ki;
    unsigned int PWMVal = (unsigned int) (fabs(total_error));
...
void SetSpeedTop(float error)  // motor speed in rad/s
{
    sum_error_t += error;
    float total_error = error*kp+sum_error_t*ki;
    unsigned int PWMVal = (unsigned int) (fabs(total_error));
